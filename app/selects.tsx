...
import { Plus, Star, Circle, CircleDot, Trash2, Clock, MessageSquare, Camera, AlertTriangle, ChevronDown, ChevronUp } from 'lucide-react-native';
  ...
  return (
    <View style={styles.starRow}>
      {[1, 2, 3, 4, 5].map(i => (
        <Star
          key={i}
          color={i <= rating ? Colors.accent.gold : Colors.bg.tertiary}
          fill={i <= rating ? Colors.accent.gold : 'transparent'}
          size={size}
        />
      ))}
    </View>
...
function SelectCard({ select, onDelete }: { select: SceneSelect; onDelete: () => void }) {
  ...
  return (
    <TouchableOpacity style={[styles.selectCard, select.isCircled && styles.circledCard]} onPress={() => setExpanded(!expanded)} activeOpacity={0.7}>
...
export default function SelectsScreen() {
  ...
  const avgRating = selects.length > 0 ? (selects.reduce((sum, s) => sum + s.rating, 0) / selects.length).toFixed(1) : '—';
  ...
  return (
    <View style={styles.container}>
      <SectionList
        ...
        renderItem={({ item }) => (
          <SelectCard select={item} onDelete={() => deleteSceneSelect(item.id)} />
        )}
          ...
          <View>
            ...
            <View style={styles.statsBar}>
              <View style={styles.statItem}>
                <Text style={styles.statValue}>{selects.length}</Text>
                <Text style={styles.statLabel}>Selects</Text>
              </View>
              <View style={styles.statDivider} />
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: Colors.accent.gold }]}>{circledCount}</Text>
                <Text style={styles.statLabel}>Circled</Text>
              </View>
              <View style={styles.statDivider} />
              <View style={styles.statItem}>
                <Text style={[styles.statValue, { color: '#60A5FA' }]}>{altCount}</Text>
                <Text style={styles.statLabel}>Alts</Text>
              </View>
              <View style={styles.statDivider} />
              <View style={styles.statItem}>
              ...
              <View style={styles.statDivider} />
              <View style={styles.statItem}>
                <Text style={styles.statValue}>{scenesWithSelects}</Text>
      ...
      <TouchableOpacity
        style={styles.fab}
        onPress={() => router.push('/new-select' as never)}
        activeOpacity={0.8}
      >
