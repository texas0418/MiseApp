...
export default function NewBudgetItemScreen() {
  ...
  const { addBudgetItem, activeProjectId, activeProject } = useProjects();
  ...
  const handleSave = useCallback(() => {
    ...
    if (!description.trim() || !estimated.trim()) { Alert.alert('Missing Info', 'Enter description and estimated amount.'); return; }
    ...
    router.back();
  }, [activeProjectId, category, description, estimated, actual, notes, vendor, paid, addBudgetItem, router]);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.projectLabel}>
        <Text style={styles.projectLabelText}>Budget for: {activeProject.title}</Text>
      </View>
        ...
        {showCategories && (
          <View style={styles.optionsList}>
            {BUDGET_CATEGORIES.map(c => (
              <TouchableOpacity key={c.value} style={[styles.option, category === c.value && styles.optionActive]} onPress={() => { setCategory(c.value as BudgetCategory); setShowCategories(false); }}>
      ...
      <View style={styles.field}>
        ...
        <TextInput style={styles.input} value={description} onChangeText={setDescription} placeholder="What is this expense for?" placeholderTextColor={Colors.text.tertiary} />
      </View>

      <View style={styles.row}>
        <View style={[styles.field, { flex: 1 }]}>
          <Text style={styles.label}>Estimated ($)</Text>
          <TextInput style={styles.input} value={estimated} onChangeText={setEstimated} placeholder="0" placeholderTextColor={Colors.text.tertiary} keyboardType="decimal-pad" />
        </View>
        <View style={{ width: 12 }} />
        <View style={[styles.field, { flex: 1 }]}>
          <Text style={styles.label}>Actual ($)</Text>
          <TextInput style={styles.input} value={actual} onChangeText={setActual} placeholder="0" placeholderTextColor={Colors.text.tertiary} keyboardType="decimal-pad" />
        </View>
      </View>

      <View style={styles.field}>
        <Text style={styles.label}>Vendor</Text>
        <TextInput style={styles.input} value={vendor} onChangeText={setVendor} placeholder="Vendor or supplier" placeholderTextColor={Colors.text.tertiary} />
      </View>

      <View style={styles.switchRow}>
        <Text style={styles.switchLabel}>Paid</Text>
        <Switch value={paid} onValueChange={setPaid} trackColor={{ true: Colors.accent.gold, false: Colors.bg.elevated }} thumbColor={Colors.text.primary} />
      </View>

      <View style={styles.field}>
        <Text style={styles.label}>Notes</Text>
        <TextInput style={[styles.input, styles.textArea]} value={notes} onChangeText={setNotes} placeholder="Additional notes" placeholderTextColor={Colors.text.tertiary} multiline numberOfLines={3} textAlignVertical="top" />
      </View>
