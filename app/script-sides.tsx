...
import { Plus, FileText, Clock, MessageSquare, Camera, Users, ChevronRight, Tag, Trash2, Eye } from 'lucide-react-native';
...
function SideCard({ side, index, onPress, onDelete, linkedShotCount }: {
  side: ScriptSide;
  index: number;
  onPress: () => void;
  onDelete: () => void;
  ...
  return (
    <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
      <TouchableOpacity style={[styles.sideCard, side.status === 'shooting-today' && styles.todayCard]} onPress={onPress} activeOpacity={0.7}>
        {/* Header */}
        ...
        {/* Synopsis */}
        <Text style={styles.synopsis} numberOfLines={3}>{side.synopsis}</Text>
...
export default function ScriptSidesScreen() {
  ...
  const filteredSides = filter === 'all' ? sides : sides.filter(s => s.status === filter);
...
const styles = StyleSheet.create({
  ...
  cardFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 8, borderTopWidth: 0.5, borderTopColor: Colors.border.subtle },
