...
export default function NewColorReferenceScreen() {
  ...
  const { activeProjectId, addColorReference } = useProjects();
  ...
  const handleSave = () => {
    ...
    addColorReference({
      id: Date.now().toString(), projectId: activeProjectId, name: name.trim(),
      sceneNumber: sceneNumber ? parseInt(sceneNumber) : undefined,
      lutStyle, primaryColor, secondaryColor, accentColor, contrast, saturation, temperature,
      referenceFilm: referenceFilm.trim() || undefined, notes: notes.trim(),
    });
    router.back();
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Stack.Screen options={{ title: 'New Color Reference' }} />
      <Text style={styles.label}>Name *</Text>
      <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="e.g. Exterior Dusk" placeholderTextColor={Colors.text.tertiary} />
      <Text style={styles.label}>Scene Number</Text>
      <TextInput style={styles.input} value={sceneNumber} onChangeText={setSceneNumber} keyboardType="number-pad" placeholder="Optional" placeholderTextColor={Colors.text.tertiary} />
      <Text style={styles.label}>LUT Style</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.lutScroll}>
        {LUT_STYLES.map(lut => (
          <TouchableOpacity key={lut.value} style={[styles.lutCard, lutStyle === lut.value && styles.lutCardActive]} onPress={() => setLutStyle(lut.value as LUTStyle)}>
            <View style={styles.lutColors}>{lut.colors.map((c, i) => <View key={i} style={[styles.lutColorBar, { backgroundColor: c }]} />)}</View>
            <Text style={[styles.lutName, lutStyle === lut.value && styles.lutNameActive]}>{lut.label}</Text>
          </TouchableOpacity>
        ))}
      </ScrollView>
      <View style={styles.colorRow}>
        <View style={styles.colorInput}><Text style={styles.label}>Primary</Text><View style={styles.colorPreview}><View style={[styles.colorDot, { backgroundColor: primaryColor }]} /><TextInput style={styles.colorField} value={primaryColor} onChangeText={setPrimaryColor} placeholderTextColor={Colors.text.tertiary} /></View></View>
        <View style={styles.colorInput}><Text style={styles.label}>Secondary</Text><View style={styles.colorPreview}><View style={[styles.colorDot, { backgroundColor: secondaryColor }]} /><TextInput style={styles.colorField} value={secondaryColor} onChangeText={setSecondaryColor} placeholderTextColor={Colors.text.tertiary} /></View></View>
        <View style={styles.colorInput}><Text style={styles.label}>Accent</Text><View style={styles.colorPreview}><View style={[styles.colorDot, { backgroundColor: accentColor }]} /><TextInput style={styles.colorField} value={accentColor} onChangeText={setAccentColor} placeholderTextColor={Colors.text.tertiary} /></View></View>
      </View>
      ...
      <TextInput style={styles.input} value={referenceFilm} onChangeText={setReferenceFilm} placeholder="e.g. Blade Runner 2049" placeholderTextColor={Colors.text.tertiary} />
      <Text style={styles.label}>Notes</Text>
      <TextInput style={[styles.input, styles.multiline]} value={notes} onChangeText={setNotes} placeholder="Color notes..." placeholderTextColor={Colors.text.tertiary} multiline />
      <TouchableOpacity style={[styles.saveBtn, !name.trim() && styles.saveBtnDisabled]} onPress={handleSave} disabled={!name.trim()}>
        <Text style={styles.saveBtnText}>Save Color Reference</Text>
      </TouchableOpacity>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  ...
  lutColorBar: { flex: 1 },
  lutName: { fontSize: 10, fontWeight: '600', color: Colors.text.tertiary, textAlign: 'center' },
  ...
  chipText: { fontSize: 13, fontWeight: '600', color: Colors.text.tertiary },
