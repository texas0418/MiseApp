...
import { Share, FileText, Camera, CalendarDays, DollarSign, ClipboardList, AlertCircle, Download } from 'lucide-react-native';
...
function generateExportText(type: string, project: any, shots: any[], schedule: any[], budget: any[], wrapReports: any[]): string {
  ...
  switch (type) {
    case 'shot-list': {
      ...
      shots.forEach(s => { if (!grouped[s.sceneNumber]) grouped[s.sceneNumber] = []; grouped[s.sceneNumber].push(s); });
      Object.entries(grouped).sort(([a], [b]) => Number(a) - Number(b)).forEach(([scene, ss]) => {
        text += `SCENE ${scene}\n`;
        ss.forEach(s => { text += `  ${s.shotNumber} | ${s.type} | ${s.movement} | ${s.lens} | ${s.status.toUpperCase()}\n  ${s.description}\n${s.notes ? `  Note: ${s.notes}\n` : ''}\n`; });
      });
    ...
    case 'schedule': {
      ...
      schedule.forEach(d => { text += `DAY ${d.dayNumber} — ${d.date}\nLocation: ${d.location}\nScenes: ${d.scenes}\nCall: ${d.callTime} | Wrap: ${d.wrapTime}\n${d.notes ? `Notes: ${d.notes}\n` : ''}${divider}\n\n`; });
      break;
    }
    case 'budget-summary': {
      text += `BUDGET SUMMARY\n\n`;
...
export default function ExportShareScreen() {
  ...
  const handleExport = async (type: string) => {
    ...
    try {
      await (await import('react-native')).Share.share({ message: text, title: `${activeProject.title} - ${type.replace('-', ' ')}` });
    } catch {
      Alert.alert('Export Ready', 'Content generated. Share functionality requires a device.');
    }
  };
      ...
      {EXPORT_OPTIONS.map(opt => {
        ...
        return (
          <TouchableOpacity key={opt.type} style={styles.card} onPress={() => handleExport(opt.type)} activeOpacity={0.7}>
            <View style={[styles.iconWrap, { backgroundColor: opt.color + '18' }]}><Icon color={opt.color} size={24} /></View>
            <View style={styles.cardText}>
              <Text style={styles.cardTitle}>{opt.title}</Text>
              <Text style={styles.cardDesc}>{opt.description}</Text>
            </View>
            <Share color={Colors.text.tertiary} size={18} />
          </TouchableOpacity>
        );
      })}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 20, fontWeight: '700', color: Colors.text.primary, marginBottom: 4 },
