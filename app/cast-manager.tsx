...
import { Plus, User, Phone, Mail, Film, Calendar, Star, Trash2, ChevronDown, ChevronUp } from 'lucide-react-native';
...
function CastCard({ member, index, onDelete }: {
  member: CastMember;
  index: number;
  onDelete: () => void;
}) {
  ...
  return (
    <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }] }}>
      <TouchableOpacity style={styles.castCard} onPress={() => setExpanded(!expanded)} activeOpacity={0.7}>
...
export default function CastManagerScreen() {
  const { activeProjectId, deleteCastMember } = useProjects();
